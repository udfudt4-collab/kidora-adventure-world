...
import { Menu, X, Compass, Gamepad2, BookOpen, Heart, Home as HomeIcon } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-xs select-none">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        {/* Right Side HUD: Stars & Hero Quick Avatar */}
          ...
          {profile && (
            <>
              ...
              <button
                type="button"
                onClick={() => handleNav('my-kidora')}
                className="btn-press hidden sm:flex items-center gap-1.5 bg-gradient-to-r from-sky-50 to-indigo-50 border border-sky-200 px-3 py-1.5 rounded-2xl text-xs font-black font-display text-sky-800 shadow-xs hover:border-sky-300"
                title="Open My Kidora Sanctuary"
              >
                <span>🧒</span>
        ...
        <div className="md:hidden bg-white border-b border-slate-200 px-4 py-4 space-y-2 animate-pop-in">
          ...
          {profile && (
            <button
              type="button"
              onClick={() => handleNav('my-kidora')}
              className="w-full py-3 px-4 rounded-2xl bg-sky-50 border border-sky-200 text-sky-800 text-sm font-black font-display text-left flex items-center justify-between"
            >
              <div className="flex items-center gap-3">
                <span className="text-xl">🧒</span>
                <span>My Kidora Sanctuary ({profile.name})</span>
              </div>
              <span className="text-xs opacity-70">→</span>
            </button>
          )}
